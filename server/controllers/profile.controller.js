...
let addProfile = async(req, res) => {
    ...
    try{
        ...
        data = profileData
        await Profile.findOneAndUpdate({phoneNumber: data.phoneNumber}, data, {upsert: true, useFindAndModify: false}, function() {
...
let updateUserDetails = async(req, res) => {
    ...
    await Profile.findOneAndUpdate({phoneNumber : phoneNumber}, {$set : updateFields}, {useFindAndModify: false})
    return res.json({
