...
let addRecord = async (req, res) => {
    ...
    await CDR.findOneAndUpdate(callerRecord, callerRecord, { upsert: true, useFindAndModify: false }, function () { })
...
// Get movement data - returns records for processing on frontend
let getMovementData = async (req, res) => {
...
// Get SIM swap detection - returns records for processing on frontend
let getSIMSwapDetection = async (req, res) => {
    let phoneNumber = req.body.phoneNumber
    if (!phoneNumber) {
        return res.json({
            message: "No phone number received",
...
// Get call pattern anomalies - returns records for processing on frontend
let getCallPatternAnomalies = async (req, res) => {
    let phoneNumber = req.body.phoneNumber
    if (!phoneNumber) {
        return res.json({
            message: "No phone number received",
...
// Get calendar activity - returns records grouped by date
let getCalendarActivity = async (req, res) => {
    let phoneNumber = req.body.phoneNumber
    if (!phoneNumber) {
        return res.json({
            message: "No phone number received",
    ...
    let startDate = req.body.startDate ? new Date(req.body.startDate) : new Date('1970-01-01')
...
let getUniqueTargets = async (req, res) => {
    try {
        const targets = await CDR.aggregate([
            {
                $group: {
                    _id: { caseNumber: '$caseNumber', callerNumber: '$callerNumber' },
                    recordCount: { $sum: 1 },
                    firstRecord: { $min: '$startTime' },
                    lastRecord: { $max: '$startTime' }
                }
            },
            {
                $group: {
        ...
        return res.status(500).json({
            message: "Error fetching unique targets",
            error: err.message,
            code: 500
        });
    }
...
// Get CDR records filtered by case number and/or caller number
let getFilteredRecords = async (req, res) => {
    try {
        const { caseNumber, callerNumber } = req.query;
        let filter = {};
        
        if (caseNumber && caseNumber !== 'all') {
            filter.caseNumber = caseNumber;
        }
        if (callerNumber && callerNumber !== 'all') {
            filter.callerNumber = callerNumber;
        }
        
        const records = await CDR.find(filter).sort({ startTime: 1 });
        return res.json(records);
    } catch (err) {
        return res.status(500).json({
...
let deleteAllRecords = async (req, res) => {
    try {
        await CDR.deleteMany({});
        return res.json({
            message: "All CDR records deleted successfully",
            code: 200
        });
    } catch (err) {
        return res.status(500).json({
            message: "Error deleting CDR records",
            error: err.message,
            code: 500
        });
    }
...
let deleteRecordsByCase = async (req, res) => {
    ...
    try {
        const result = await CDR.deleteMany({ caseNumber: caseNumber });
        return res.json({
...
module.exports = {
    ...
    getSIMSwapDetection: getSIMSwapDetection,
    getCallPatternAnomalies: getCallPatternAnomalies,
    getCalendarActivity: getCalendarActivity,
    deleteAllRecords: deleteAllRecords,
    deleteRecordsByCase: deleteRecordsByCase,
    getUniqueTargets: getUniqueTargets,
