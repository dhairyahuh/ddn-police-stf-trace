...
const parseDateTime = (dateStr, timeStr) => {
    try {
        ...
        // Use UTC to match server parsing
...
db.once('open', async () => {
    ...
    dataStream
        ...
        .on('data', (row) => {
            ...
            // Parse duration more carefully
            let duration = parseInt(durationStr.toString().replace(/['"]/g, '').trim());
        ...
        .on('end', async () => {
            ...
            // Include duration in key to handle records with same caller/called/time but different duration
            const dbMap = new Map();
            dbRecords.forEach(record => {
                const key = `${record.callerNumber}_${record.calledNumber}_${record.startTime.getTime()}_${record.callDuration}`;
            ...
            for (const csvRecord of csvRecords) {
                const key = `${csvRecord.callerNumber}_${csvRecord.calledNumber}_${csvRecord.startTime.getTime()}_${csvRecord.callDuration}`;
